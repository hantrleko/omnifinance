"""简单移动平均线交叉策略回测器 (MA Crossover Backtester)

SMA 金叉/死叉策略，全仓模拟，Plotly 可视化，Streamlit 前端。
"""
...
if len(data) < sma_long:
    st.error(f"数据量不足：仅有 {len(data)} 天，但长期 SMA 需要 {sma_long} 天。请缩短 SMA 窗口或扩大日期范围。")
    st.stop()
...
fig_equity.add_trace(go.Scatter(
    ...
    hovertemplate="%{x|%Y-%m-%d}<br>持有: ¥%{y:,.0f}<extra></extra>",
))
...
if trades_df.empty:
    st.info("当前参数下未产生任何交易信号。请尝试调整 SMA 窗口或日期范围。")
else:
...
# ── 页脚 ──────────────────────────────────────────────────
st.divider()
st.caption("MA 交叉策略回测器 | 数据来源：Yahoo Finance | 运行：`streamlit run app.py`")
