...
if irr >= alt_return_pct:
    st.success(f"结论：保单 IRR（{irr:.2f}%）≥ 替代投资收益（{alt_return_pct:.1f}%），储蓄型保险具有竞争力。")
    st.caption("下一步：可考虑将其作为稳健型低风险资产配置的一部分。")
elif irr >= 2.0:
    st.info(f"结论：保单 IRR（{irr:.2f}%）低于替代投资（{alt_return_pct:.1f}%），但仍为正收益。")
    st.caption(f"说明：选择此产品的机会成本约为 {fmt(alt_gap, decimals=0)}（期末与替代投资的差距）。")
    st.caption("下一步：综合考量保障需求与流动性后再决策，不宜单纯以收益率比较。")
else:
    st.warning(f"结论：保单 IRR（{irr:.2f}%）偏低，储蓄属性较弱，主要价值在于保障功能。")
    st.caption(f"说明：若目的为纯储蓄增值，替代投资可获得约 {fmt(alt_gap, decimals=0)} 的额外收益。")
    st.caption("下一步：评估是否有更高 IRR 的同类产品，或考虑将储蓄与保障分离配置。")
...
# ── 页脚 ──────────────────────────────────────────────────
st.divider()
st.caption("🛡️ 保险产品测算器 | 仅供学习参考，不构成保险或投资建议 | 运行：`streamlit run app.py`")
