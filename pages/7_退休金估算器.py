...
track_recent_page(st.session_state, 'retirement')
...
with st.expander("🔧 配置提款策略模拟", expanded=False):
    st.markdown(f"以退休时资产 **{fmt(result.projected_at_retire, decimals=0)}** 为起点，模拟不同提款策略下的资产变化。")
    ...
    if _ws_assets > 0 and _ws_years > 0:
        ...
        final_ws_rows = []
        for strategy_name in color_map:
            subset = ws_df[ws_df["策略"] == strategy_name]
