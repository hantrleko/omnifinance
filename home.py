...
with st.expander("📋 版本历史", expanded=False):
    st.markdown("""
### v2.3.0 — 财报学习、链式计算与稳定性升级
📚 财务报表分析模块（第1-6模块）重构：学习区与联系区结构更清晰，题目类型按“学习/练习”链路完整区分，覆盖考点与公式解题步骤。
🧩 点击左侧“配套练习”可自动跳转到对应练习区；侧边栏锚点与页面内导航同步修复。
🧪 所有后续模块默认新增“偏计算+链条题（案例导向）”，从题面到公式再到结论逐步闭环。
🛡️ 稳定性修复：关键路径异常处理增强（含行情/缓存异常场景回退），减少页面白屏与中断概率。
